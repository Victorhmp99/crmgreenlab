...
export function RegisterPage() {
  ...
  async function onSubmit(data: RegisterForm) {
    try {
      setError(null)
      const result = await registerTenant(data.clinicName, data.email, data.password)

      if (result.needsEmailConfirmation) {
        // Supabase exige confirmação de e-mail — avisa o usuário
        setEmailSent(true)
      } else {
        // Conta criada e sessão ativa — vai direto pro dashboard
        navigate('/dashboard')
      }
    } catch (err) {
      ...
      <AuthLayout>
        <div className="flex flex-col items-center text-center gap-5 py-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold mt-1" style={{ color: '#e8e8e8' }}>
              {getValues('email')}
            </p>
          </div>
          ...
          <div className="rounded-xl px-4 py-3 text-xs text-left w-full"
            ...
            <p className="font-semibold mb-1" style={{ color: '#00e676' }}>Próximos passos</p>
            <ol className="list-decimal ml-4 space-y-1" style={{ color: '#00a846' }}>
              <li>Abra o e-mail que enviamos</li>
              <li>Clique no link de confirmação</li>
              <li>Você será redirecionado ao Green Hub</li>
            </ol>
          </div>
          ...
          <div className="rounded-xl px-4 py-3 text-xs text-left w-full"
            ...
            <p className="font-medium" style={{ color: '#fbbf24' }}>
              ⚡ Dica: Para pular a confirmação de e-mail durante o desenvolvimento, desative-a no painel
    ...
    <AuthLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: '#e8e8e8' }}>
          Criar conta no Green Hub
        </h1>
        <p className="text-sm mt-1" style={{ color: '#666' }}>
          Configure sua clínica em menos de 1 minuto
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="relative">
            <Building2 size={15} className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"
              style={{ color: errors.clinicName ? '#ff4444' : '#444' }} />
            <input
              type="text"
              ...
              style={{
                background: '#1a1a1a',
                border: `1px solid ${errors.clinicName ? '#ff4444' : '#2a2a2a'}`,
                color: '#e8e8e8',
              }}
              onFocus={(e) => {
                if (!errors.clinicName) e.currentTarget.style.border = '1px solid var(--tenant-primary)'
              }}
              {...register('clinicName')}
            />
          </div>
          {errors.clinicName && (
            <p className="text-xs" style={{ color: '#ff4444' }}>{errors.clinicName.message}</p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            onFocus={(e) => {
              if (!errors.confirm) e.currentTarget.style.border = '1px solid var(--tenant-primary)'
            }}
            {...register('confirm')}
          />
          {errors.confirm && (
            <p className="text-xs" style={{ color: '#ff4444' }}>{errors.confirm.message}</p>
          )}
        </div>
